			...
			const char *prefix, unsigned int plen)
{
	...
	/*ndpi id*/
	ndpi_cnt++;
	NLA_PUT_BE32(inst->skb, NFULA_NDPI_ID, &ndpi_cnt);
	if (unlikely(ndpi_cnt > 1234)){
		printk(KERN_WARNING "nfnetlink_log: reset ndpi_cnt\n")
		ndpi_cnt = 0;
	}
	/* UID */
	if (skb->sk) {
		  ...
		  const char *prefix)
{
	...
	size =    NLMSG_SPACE(sizeof(struct nfgenmsg))
		...
		+ nla_total_size(sizeof(u_int32_t)) /*custom*/
